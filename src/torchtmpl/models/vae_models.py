# Implement VAE and fully-convolutional VAE architectures for wireframe generation.
...
class MaskedGroupNorm(nn.Module):
    ...
    def forward(self, x, mask):
        if mask is None:
            ...
            # Standard GroupNorm path when there is no padding mask.
        ...
        # Normalize per group while ignoring padded regions.
        x_g = x.view(B, G, C // G, H, W)
        mask_g = mask.view(B, 1, 1, H, W)
    ...
    def forward(self, x, mask=None):
        ...
        for size in self.pool_sizes:
            if mask is None:
                ...
                # Divide pooled activations by pooled mask to keep true averages.
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # Shortcut branch for channel/stride alignment when needed.
        identity = x
        if self.use_projection:
...
class VAE(nn.Module):
    ...
    def decode(self, z: torch.Tensor, target_size: Optional[tuple] = None) -> torch.Tensor:
        ...
        # Project latent vector to a small spatial seed feature map.
        d = self.fc_decode(z)
        ...
        # Progressive upsampling back to image resolution.
        d = self.dec_up1(d)
        d = self.dec_block1(d)
    ...
    def _encode(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> tuple:
        ...
        # Multi-stage convolutional encoder with spatial downsampling.
...
class FullyConvVAE(nn.Module):
    ...
    def _encode(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> tuple:
        ...
        # Encode into a spatial latent grid (not a single vector).
        e1 = self.enc_conv1_conv(x)
        e1 = self.enc_conv1_gn(e1, mask)
        e1 = self.enc_conv1_relu(e1)
    ...
    def decode(self, z: torch.Tensor, mask: Optional[torch.Tensor] = None, target_size: Optional[tuple] = None) -> torch.Tensor:
        # Decode latent grid back to image space with nearest-neighbor upsampling.
        d = self.dec_in(z)
    ...
    def sample(self, num_samples: int = 1, device: torch.device = None, output_size: tuple = (1024, 128)) -> torch.Tensor:
        """
        For a fully convolutional VAE, sample latent noise with spatial dimensions
        derived from the output size downsampled by a factor of 16.
        """
        if device is None:
            device = next(self.parameters()).device
