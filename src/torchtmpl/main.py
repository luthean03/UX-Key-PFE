# Entry-point orchestration for training, evaluation, interpolation, and clustering commands.
...
def train(config):
    ...
    # 1) Fix random seeds for reproducible runs.
    seed = config.get("data", {}).get("seed", 42)
    utils.set_reproducibility(seed)


    # 2) Expand relative config paths once at startup.
    ...
    # 3) Build train/validation dataloaders.
    ...
    # 5) Optionally resume model weights from a checkpoint.
    ...
    # 6) Build loss/optimizer objects.
    logging.info("= Loss")
    ...
    scheduler = None
    _sched_deferred = True


    # 7) Create run directory and tensorboard writer.
    ...
    # Keep an exact config snapshot with each run.
    ...
    # Write a human-readable run summary (config, model, datasets).
    summary_text = (
        f"Logdir : {logdir}\n"
        + "## Command \n"
        + " ".join(sys.argv)
    ...
    # Track and save the best checkpoint based on validation loss.
    model_checkpoint = utils.ModelCheckpoint(model, str(logdir / "best_model.pt"), min_is_best=True)


    # Configure criterion and KL warmup schedule.
    ...
    # Enable AMP when requested and CUDA is available.
    ...
    # SSIM is computed on validation reconstructions for quality monitoring.
    ssim_metric = StructuralSimilarityIndexMeasure(data_range=1.0).to(device)


    # Some schedulers (OneCycleLR) need steps_per_epoch computed from loaders.
    if _sched_deferred:
    ...
    for e in range(config["nepochs"]):
        ...
        # Linearly ramp KL weight during warmup epochs.
        ...
        for i, (inputs, targets, masks) in enumerate(pbar):
            ...
            # Accumulate gradients to emulate a larger effective batch size.
        ...
        # Flush remaining gradients when batch count is not a multiple of accumulation steps.
        if (i + 1) % grad_accumulation_steps != 0:
            if use_amp:
                scaler.unscale_(optimizer)
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=grad_clip_max_norm)
    ...
    # ---- Validation pass ----
        ...
        # Save best checkpoint according to validation ELBO.
        ...
        # Save one side-by-side reconstruction preview per epoch.
        try:
            with torch.no_grad():
                ...
                idx = 0
                tgt = sample_targets[idx].cpu()
                rec = sample_recon[idx].cpu()
                msk = sample_masks[idx].cpu()
                ...
                comparison = torch.stack([tgt, rec])
        ...
        # Push scalar metrics to TensorBoard when enabled.
        if writer is not None:
...
def test(config):
    ...
    # Process each image independently and export a comparison panel.
    with torch.no_grad():
        for idx, img_path in enumerate(tqdm(image_files, desc="Processing images")):
            try:
                ...
                # Pad to the model stride so encoder/decoder shapes stay consistent.
                stride = 32
                ...
                grid_width = original_pil.width + recon_pil.width + 20
...
def interpolate(config):
    ...
    # Compute a common padded canvas so both images map to comparable latent shapes.
    max_h, max_w = 0, 0
    ...
    # Build output strip: original1, recon1, interpolation frames, recon2, original2.
    interp_frames = []
...
def clustering(config):
    ...
    with torch.no_grad():
        for img_path in tqdm(image_files, desc="Encoding images"):
            try:
                ...
                images.append(img_tensor[0].cpu())
    ...
    # Cluster in full latent space, then only subsample for plotting.
